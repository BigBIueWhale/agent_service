...
pub struct AgentLock {
    ...
    pub output_language_sha256: String,
...
fn validate_lock(lock: &StackLock) -> ServiceResult<()> {
    ...
    if [
        ...
        lock.agent.output_language_sha256.as_str(),
    ...
    if lock.backend.container_name != "qwen38-agent-native"
        ...
        || lock.backend.image_tag != "qwen38-vllm:qwen38-27b-nvfp4-k8v4-runtime-v26"
        || lock.backend.image_id
            != "sha256:dcc869c00e726abd801d3217ef5acb9350e8e1044085ebc0fbcb6c5d8eac9210"
    ...
    if lock.backend.user != "2000:0"
        ...
        || lock.backend.cache_volume != "qwen38-vllm-cache-socket-isolated-nonroot-vision-agent-v21"
    ...
    {
        return fail(
            "backend environment differs from the strict non-root v21 runtime contract".into(),
        );
    }
