...
fn validate_policy(policy: &Policy) -> Result<(), String> {
    if policy.schema_version != 1
        ...
        || policy.backend_cache_volume
            != "qwen38-vllm-cache-socket-isolated-nonroot-vision-agent-v18"
    ...
    if policy.broker.image_tag != "qwen38-docker-broker:1.1.0"
        ...
        || policy.agent.image_tag != "qwen38-agent:0.21.12-b965d5f8-v9"
...
async fn execute(request: Request, policy: &Policy) -> Result<Value, String> {
    match request {
        ...
        Request::WaitAgentReady { session_id } => {
            ...
            .await?;
            let ready = parse_agent_ready(&line, policy)?;
            Ok(json!({
