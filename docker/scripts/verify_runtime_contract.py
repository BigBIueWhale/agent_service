...
def verify_wrapper(contract: dict[str, Any], wrapper: str) -> None:
    ...
    if re.search(r"QWEN_STREAM_(IDLE_TIMEOUT|MAX_LIFETIME)_MS", wrapper):
        raise ContractError(
            "agent wrapper must not set stream bounds: the client derives every one of them "
            "(the request timeout, the idle bound, max_tokens at the decode floor), and no "
            "setting carries them"
        )
